...
This workflow is applicable even if you do not need/want to bin the assembled metagenomic contigs. In that case, your reference/index will be the dereplicated contigs generated from `2b.post_assembly` and not the co-assemblies. The per-sample coverage profiles based on dereplicated contigs can provide relative abundance information of genes of interest that are useful for analyses (e.g. in relation to transcriptome, if available).
