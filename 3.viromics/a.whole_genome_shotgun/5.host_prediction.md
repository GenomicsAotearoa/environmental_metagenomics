...
Modify the above for any number of sequences. You may need to run 
- this as a **SLURM** job due to file size as `sed` requires the entire file to be read into memory while processing
- modify this into a loop (`for` or `while`, depending on your standard error reporting for **Crass**) 
- propagate the process using `xargs`
...
# Working dir
mkdir -p /working/dir/host_prediction/tRNA_blast/aragorn_out
cd /working/dir/host_prediction/tRNA_blast
...
# Concatenate bins into all.hosts.fna
mkdir -p host_prediction/pairwise_blast/infiles
cat /path/to/bin_files/*.fa > host_prediction/pairwise_blast/infiles/all.hosts.fna
...
makeblastdb \
-in host_prediction/pairwise_blast/infiles/all.hosts.fna -dbtype nucl \
-out host_prediction/pairwise_blast/infiles/all.hosts.fna
...
-db host_prediction/pairwise_blast/infiles/all.hosts.fna \
-outfmt "6 qseqid qlen sseqid slen pident length mismatch gapopen qstart qend sstart send evalue bitscore qcovs" \
-perc_identity 70 \
-out host_prediction/pairwise_blast/vOTUs.pairwise_blast.txt
```
